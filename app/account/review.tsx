...
export default function UnderReviewScreen() {
  ...
  const isApproved = state.profileStatus === 'approved' || (state.accountStep >= 7 && state.documentsStatus === 'submitted');
    ...
    const check = async (silent = true) => {
      ...
      try {
        ...
        if (status === 'approved') {
          ...
          router.replace('/(tabs)');
        } else if (status === 'rejected' || docsStatus === 'rejected' || accountStep < 7) {
          router.replace(accountStep >= 6 ? '/account/pricing' : '/account/documents');
        }
      } catch {
        // keep local state
      } finally {
      ...
      <View style={styles.content}>
        ...
        {isRejected ? (
          <TouchableOpacity
            ...
            onPress={() => router.replace(state.accountStep >= 6 ? '/account/pricing' : '/account/documents')}
          >
            <Text style={styles.secondaryText}>{state.accountStep >= 6 ? 'Mettre a jour mes tarifs' : 'Mettre a jour mes documents'}</Text>
          </TouchableOpacity>
        ) : null}
      </View>
    </SafeAreaView>
